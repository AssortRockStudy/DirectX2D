...
int CTexture::Load(const wstring& _strFilePath)
{
	...
	// 2. ScratchImage에 있는 데이터를 ID3D11Texture2D 객체로 전달
	// 3. ID3D11Texture2D를 이용한 ShaderResourceView만들기
	...
	//만들어진 Shader Resource View를 이용해서 생성된 ID3D11Texture2d 객체를 알아낸다.
	m_SRV->GetResource((ID3D11Resource**)m_Tex2D.GetAddressOf());


	// ID3D11Textrue2D 객체를 통해서 생성될때 사용된 옵션(DESC) 정보를 가져온다.
