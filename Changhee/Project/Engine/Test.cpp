...
int TestInit()
{
	...
	tBufferDesc.StructureByteStride = sizeof(UINT);
	...
	// 인덱스 버퍼 생성
	if (FAILED(DEVICE->CreateBuffer(&tBufferDesc, &tSubData, g_IB.GetAddressOf())))
	{
	...
	tBufferDesc.StructureByteStride = sizeof(tTransform);
