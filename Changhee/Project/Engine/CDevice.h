...
private:
	...
	ComPtr<IDXGISwapChain>			m_SwapChain;		// SwapChain(Ãâ·Â ¹öÆÛ ÁöÁ¤)

	//OM(OutputMergeState)
	ComPtr<ID3D11Texture2D>			m_RTTex;			// ·»´õÅ¸°Ù ÅØ½ºÃÄ
	ComPtr<ID3D11RenderTargetView>	m_RTView;			// ·»´õÅ¸°Ù ºä

	ComPtr<ID3D11Texture2D>			m_DSTex;			// ‰X½º ½ºÅÙ½Ç ÅØ½ºÃÄ
	ComPtr<ID3D11DepthStencilView>	m_DSView;			// ‰X½º ½ºÅÙ½Ç ºä
