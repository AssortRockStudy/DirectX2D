...
void CS_ParticleUpdate(uint3 id : SV_DispatchThreadID)
{
    ...
    else
    {
        ...
        if (Module.arrModuleCheck[4])
        {
            ...
            {
                Particle.vNoiseForce = normalize(vRand.xyz * 2.f - 1.f) * Module.NoiseForce.NoiseForceScale;
                Particle.NoiseForceTime = g_time;
            }
            else if (Module.NoiseForce.NoiseForceTerm < g_time - Particle.NoiseForceTime)
            {
                Particle.vNoiseForce = normalize(vRand.xyz * 2.f - 1.f) * Module.NoiseForce.NoiseForceScale;
                Particle.NoiseForceTime = g_time;
            }
